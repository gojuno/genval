...
func (r DogsMapAlias) Validate() error {
	...
	for k, v := range r {
		...
		if err := v.Validate(); err != nil {
			errs.AddField(fmt.Sprintf("r.%v", k), err)
		}
	}
	return errs.ErrorOrNil()
}
...
func (r User) Validate() error {
	...
	for k, v := range r.Dict {
		...
		if utf8.RuneCountInString(k) > 64 {
			errs.AddFieldf(fmt.Sprintf("Dict.%v", k), "longer than 64 chars")
		}
		if v < -35 {
			errs.AddFieldf(fmt.Sprintf("Dict.%v", k), "less than -35")
		}
		if v > 34 {
			errs.AddFieldf(fmt.Sprintf("Dict.%v", k), "more than 34")
		}
	}
	for k, v := range r.DictDogs {
		...
		if err := v.ValidateOptional(); err != nil {
			errs.AddField(fmt.Sprintf("DictDogs.%v", k), err)
		}
	...
	for k, v := range r.MapOfMap {
		...
		_ = v
		if len(v) < 1 {
			errs.AddFieldf(fmt.Sprintf("MapOfMap.%v", k), "less items than 1")
		}
		for k, v := range v {
			_ = k
			_ = v
			if utf8.RuneCountInString(v) < 3 {
				errs.AddFieldf(fmt.Sprintf("v.%v", k), "shorter than 3 chars")
			}
		}
	}
